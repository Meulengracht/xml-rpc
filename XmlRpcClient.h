...
// XmlRpc++ Copyright (c) 2016 by Philip Meulengract
//
#if defined(_MSC_VER)
...
namespace XmlRpc {
  ...
  public:
    ...
    //! This is automatically called if the port is 443
    void enableSSL();
#endif

    //! Can be used to test the connection, returns true if
    //! the connection can be established.
    bool testConnection() { return setupConnection(); }
...
#ifdef _OPENSSL_ENABLED
    // The SSL context handle
    void *_sslHandle;

    // Whether or not to cleanup the SSL library
    bool _cleanupSSL;
#endif
